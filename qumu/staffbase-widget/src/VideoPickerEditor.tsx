...
function thumbUrl(v: VideoItem): string {
  ...
  return `${THUMBNAIL_FALLBACK_BASE_URL}${v.id}/640/360`;
}
...
export default function VideoPickerEditor({ onSelect, onCancel }: Props) {
  ...
  // Filter selection format: "<fieldTitle>|<optionGuid>|<optionValue>"
  ...
  // Some tenants expose location-like values under "Store Department" instead of "Location".
  useEffect(() => {
    fetchMasterData(['Division', 'Location', 'Store Department'])
  ...
  useEffect(() => {
    ...
    if (selectedFilter) {
      ...
      const optGuid = parts[1] || '';
      const optValue = parts[2] || '';
      const metaType = metaRef.current.find(m => m.title === fieldTitle);
      if (metaType && optGuid) {
        rules.push({ fieldGuid: metaType.guid, fieldTitle, optionGuid: optGuid, optionValue: optValue });
      }
    }
  ...
  }, [offset, search, selectedFilter, retryCount]);
  ...
  const locationMeta =
    metaTypes.find(m => m.title === 'Location') ||
    metaTypes.find(m => m.title === 'Store Department');
  ...
  function handleSearch(val: string) {
    ...
    searchDebounce.current = setTimeout(() => {
      ...
      setOffset(0);
      setSelVideo(null);
      if (val) { setSelectedFilter(''); } // search clears filter
    }, 300);
  }
  ...
  function handleFilterChange(nextFilter: string) {
    ...
    setOffset(0);
    setSelVideo(null);
    if (nextFilter) { setSearch(''); setInputValue(''); } // filter clears search
  }
    ...
    <div style={S.root} onClick={stop} onMouseDown={stop} onKeyDown={stop} onKeyUp={stop}>
      ...
      <div style={S.toolbar}>
        ...
        >
          ...
          {(divisionMeta?.options || []).filter(opt => opt.value !== 'All Divisions/Locations').length > 0 && (
            <optgroup label="Divisions">
              {(divisionMeta?.options || [])
                .filter(opt => opt.value !== 'All Divisions/Locations')
                .map(opt => (
                  <option key={`division-${opt.guid}`} value={`Division|${opt.guid}|${opt.value}`}>{opt.value}</option>
                ))}
            </optgroup>
          )}
          {(locationMeta?.options || []).filter(opt => opt.value !== 'All Divisions/Locations').length > 0 && (
            <optgroup label="Locations">
              {(locationMeta?.options || [])
                .filter(opt => opt.value !== 'All Divisions/Locations')
                .map(opt => (
                  <option key={`location-${opt.guid}`} value={`${locationMeta?.title || 'Location'}|${opt.guid}|${opt.value}`}>{opt.value}</option>
                ))}
            </optgroup>
          )}
        </select>
